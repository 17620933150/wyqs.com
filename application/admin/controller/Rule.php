...
class Rule extends Common {
    ...
    function rulelst() {
        ...
        if ($cateid !== null || $search !== null ) {
            if ($cateid !== '0') {
                $rule = db('sh_rule')
                    ...
                    ->where("a.rule_name","like","%".$search."%")
                    ->where('a.rule_roles',$cateid)
        ...
        return view('rule/rulelst',['rule'=>$rule,'roles'=>$roles,'cateid'=>$cateid,'search'=>$search]);
    }
    ...
    function ruleadd() {
        ...
        $roles = db('sh_roles')->select();
        return view('rule/ruleadd',['roles'=>$roles]);
    }
